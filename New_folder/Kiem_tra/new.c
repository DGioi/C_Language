...
Node* root[M], *curr, *changelog, *alllog;
...
void load(){
    ...
    while(!feof(f)){
        ...
        alllog = insert(alllog, bsx, date, time, inout);
...
void in(char* bsx){
    ...
    if(r==NULL){
        ...
        alllog = insert(alllog, bsx, date, time, 1);
        int idx = h(date);
...
void out(char* bsx){
    ...
    if(r!=NULL){
        ...
        alllog = insert(alllog, bsx, date, time, 0);
        int idx = h(date);
        root[idx] = insert(root[idx], bsx, date, time, 0);
    }else printf("Xe khong o trong bai!\n");
}

Node* search(Node* r, char* bsx){
    if(r == NULL) return NULL;
    int c = strcmp(r->bsx,bsx);
    if(c == 0){
        printf("%s %s %s %d\n",r->date,r->time,r->bsx,r->inout);
        search(r->leftChild,bsx);
        search(r->rightChild,bsx);
        return r;
    } 
    if(c < 0) return search(r->rightChild,bsx);
    return search(r->leftChild,bsx);
}
...
void main(){
    while(1){
        ...
        else if(strcmp(cmd,"6")==0) processStoreChange(); 
        else if(strcmp(cmd,"7")==0) processStoreCurr();
        else if(strcmp(cmd,"8")==0) processSearch();
        else if(strcmp(cmd,"9")==0) printList();
    }
    for(int i = 0; i < M; i++)
        freeTree(root[i]);
}
